...
export const SHAREPOINT_SITE_URL = 'https://[yourtenant].sharepoint.com/sites/SkillBridge';
export const SHAREPOINT_HOSTNAME = '[yourtenant].sharepoint.com';
export const SHAREPOINT_SITE_PATH = '/sites/SkillBridge';
...
export function getAzureObjectId(account) {
  return account?.idTokenClaims?.oid || account?.localAccountId || account?.homeAccountId || '';
}
...
export async function getListIdByName(listName) {
  if (listIdCache.has(listName)) {
    return listIdCache.get(listName);
  }

  const siteId = await getSiteId();
  const response = await graphClient.api(`/sites/${siteId}/lists`).get();
  const list = response.value.find((entry) => entry.displayName === listName);
...
export async function getUserByUserId(userId) {
  const siteId = await getSiteId();
  const usersListId = await getListIdByName(LIST_NAMES.USERS);
...
export async function ensureUserRegistered(account) {
  ...
  if (!userId) {
    throw new Error('Unable to resolve Azure AD object ID for current account.');
  }
  ...
  if (!existing.value.length) {
    const created = await graphClient.api(`/sites/${siteId}/lists/${usersListId}/items`).post({
      fields: {
        UserID: userId,
